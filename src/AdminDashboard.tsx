...
const boyDefaultImage = `${(import.meta as any).env.BASE_URL || "/"}boy.png`;
const girlDefaultImage = `${(import.meta as any).env.BASE_URL || "/"}girl.png`;
...
export default function AdminDashboard({
  ...
  const [resetCodeConfirmId, setResetCodeConfirmId] = useState<string | null>(
    null,
  );
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!isAdmin(result.user.email)) {
        setLoginError(
          "您登入的 Google 帳號非管理員帳號！請登出並改用管理員帳號。",
        );
      } else {
  ...
  const confirmResetCode = async () => {
    ...
    try {
      await setDoc(
        doc(db, "inviteCodes", resetCodeConfirmId),
        {
          used: false,
          usedBy: "",
        },
  ...
  const drawWinners = () => {
    ...
    const animInterval = setInterval(() => {
      ...
      if (iterations >= maxIterations) {
        ...
        const drawn = shuffled.slice(
          0,
          Math.min(siteConfig.winnerCount, correct.length),
        );
        setWinners(drawn);
  ...
  const confirmDeleteGuess = async () => {
    ...
    try {
      ...
      if (guessToDelete && guessToDelete.inviteCode) {
        await setDoc(
          doc(db, "inviteCodes", guessToDelete.inviteCode),
          {
            used: false,
            usedBy: "",
          },
  ...
  const exportCSV = () => {
    ...
    const rows = guesses.map((item) => {
      const isCorrect = siteConfig.actualGender
        ? item.gender === siteConfig.actualGender
          ? "猜對"
          : "未中"
        : "尚未揭曉";
      return [
  ...
  const correctGuesses = siteConfig.actualGender
    ? guesses.filter((g) => g.gender === siteConfig.actualGender)
    : [];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#faf6f0]">
        <div className="text-center">
          ...
          <div className="text-[#8c7a6b] font-extrabold text-sm">
            正在驗證身分中...
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#faf6f0] to-[#f4ebe1] p-4 font-sans">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-[0_24px_48px_rgba(176,142,114,0.15)] border border-[#eedac5] overflow-hidden">
          <div className="p-8 text-center border-b border-[#f5ebe2]">
            ...
            <h2 className="text-2xl font-bold text-[#524339] mb-1.5 font-sans">
              管理員登入
            </h2>
          ...
          <form onSubmit={handleEmailLogin} className="p-8 space-y-5">
            <div>
              <label className="block text-xs font-bold text-[#8c7a6b] uppercase tracking-wider mb-2">
                管理者電子信箱
              </label>
              <input
                type="email"
            ...
            <div className="relative my-6 flex items-center justify-center">
              <span className="absolute bg-white px-3 text-xs text-[#8c7a6b] font-bold">
                或使用 Google 登入
              </span>
          ...
          <div className="bg-[#faf6f0] p-5 border-t border-[#eedac5] text-center">
            <a
              href="#/"
              className="text-xs font-bold text-[#b08e72] hover:underline"
            >
              ← 返回首頁投票
            </a>
          </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#faf6f0] to-[#f4ebe1] p-4 font-sans">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-[0_24px_48px_rgba(176,142,114,0.15)] border border-[#eedac5] overflow-hidden p-8 text-center">
          ...
          <p className="text-[#8c7a6b] text-sm leading-relaxed mb-6">
            您的登入帳號為{" "}
            <span className="font-extrabold text-[#524339]">
              {currentUser.email}
            </span>
            。<br />
            本後台僅限{" "}
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-[var(--color-primary-dark)]">
        載入中...
      </div>
    );
    ...
    <div className="w-[min(1320px,calc(100%-32px))] mx-auto py-6 pb-10 relative z-10 text-[var(--color-text)]">
      <div className="flex justify-between items-center gap-4 flex-wrap mb-6">
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <a
            href="#/"
            className="bg-white dark:bg-slate-900 text-[var(--color-primary-dark)] border border-[rgba(140,111,232,.12)] px-4 py-2.5 rounded-full text-sm font-extrabold hover:-translate-y-px transition-transform shadow-sm no-underline"
          >
            回首頁
          </a>
          <button
            onClick={loadData}
            className="bg-white dark:bg-slate-900 text-[var(--color-primary-dark)] border border-[rgba(140,111,232,.12)] px-4 py-2.5 rounded-full text-sm font-extrabold hover:-translate-y-px transition-transform shadow-sm"
          >
            重新整理資料
          </button>
          <button
            onClick={exportCSV}
            className="bg-gradient-to-br from-[var(--color-primary)] to-[#aa91ff] text-white shadow-[0_12px_26px_rgba(140,111,232,.25)] px-4 py-2.5 rounded-full text-sm font-extrabold hover:-translate-y-px transition-transform"
          >
            匯出 CSV
          </button>
          <button
            onClick={handleLogout}
            className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 px-4 py-2.5 rounded-full text-sm font-extrabold hover:-translate-y-px transition-transform shadow-sm"
          >
            登出系統 🚪
          </button>
        </div>
      ...
      <section
        className="border border-[var(--color-glass-border)] shadow-[var(--shadow-custom)] rounded-[var(--radius-xl)] p-5 mb-5 backdrop-blur-sm"
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="bg-white/10 dark:bg-slate-900/60 rounded-[20px] p-4 border border-[var(--color-glass-border)]">
            ...
            <div className="text-[var(--color-primary-dark)] text-3xl font-extrabold">
              {guesses.length}
            </div>
          </div>
          <div className="bg-white/10 dark:bg-slate-900/60 rounded-[20px] p-4 border border-[var(--color-glass-border)]">
      ...
      >
        <div className="flex justify-between items-center gap-4 flex-wrap mb-4">
          ...
          <button
            ...
            className="bg-gradient-to-br from-[#34b57b] to-[#4fcb92] text-white px-4 py-2.5 rounded-full text-sm font-extrabold hover:-translate-y-px transition-transform"
          >
            儲存網站設定
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="flex flex-col gap-2 mb-3">
            ...
            <input
              id="closeTime"
              value={toDatetimeLocalValue(siteConfig.closeTime)}
              onChange={(e) =>
                setSiteConfig((prev) => ({
                  ...prev,
                  closeTime: new Date(e.target.value).toISOString(),
                }))
              }
              className="w-full border border-[rgba(140,111,232,.15)] bg-white/10 dark:bg-slate-900/60 rounded-2xl px-4 py-3.5 text-[15px] text-[var(--color-text)] outline-none"
              type="datetime-local"
            />
          </div>
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            <select
              id="isVotingOpen"
              value={String(siteConfig.isVotingOpen)}
              onChange={handleInputChange}
              className="w-full border border-[rgba(140,111,232,.15)] bg-white/10 dark:bg-slate-900/60 rounded-2xl px-4 py-3.5 text-[15px] text-[var(--color-text)] outline-none cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            >
              ...
              <option value="男寶" className="text-black">
                男寶
              </option>
              <option value="女寶" className="text-black">
                女寶
              </option>
            </select>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            <input
              id="boyImageUrl"
              value={siteConfig.boyImageUrl || ""}
              onChange={handleInputChange}
              className="w-full border border-[rgba(140,111,232,.15)] bg-white/10 dark:bg-slate-900/60 rounded-2xl px-4 py-3 text-[13px] text-[var(--color-text)] outline-none font-mono"
              placeholder="男寶照片路徑或網址"
            />
            {siteConfig.boyImageUrl && (
              <div className="flex items-center gap-2 mt-1">
                ...
                <img
                  src={siteConfig.boyImageUrl}
                  alt="Boy Preview"
                  className="w-10 h-10 object-cover rounded-lg border border-[var(--color-glass-border)] bg-slate-100"
                />
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            <input
              id="girlImageUrl"
              value={siteConfig.girlImageUrl || ""}
              onChange={handleInputChange}
              className="w-full border border-[rgba(140,111,232,.15)] bg-white/10 dark:bg-slate-900/60 rounded-2xl px-4 py-3 text-[13px] text-[var(--color-text)] outline-none font-mono"
              placeholder="女寶照片路徑或網址"
            />
      ...
      <section
        className="border border-[var(--color-glass-border)] shadow-[var(--shadow-custom)] rounded-[var(--radius-xl)] p-5 mb-5 backdrop-blur-sm relative overflow-hidden"
        style={{ background: "var(--color-glass-bg)" }}
      >
        {drawAnimating && (
            ...
            <div className="text-center transform scale-110">
              ...
              {currentAnimGuess && (
                <div className="bg-white/10 border-2 border-yellow-400/50 rounded-3xl p-8 shadow-[0_0_50px_rgba(250,204,21,0.4)] transition-all">
                  <div className="text-xl text-yellow-100 font-bold mb-2">
                    候選人
                  </div>
                  <div className="text-5xl font-extrabold text-white mb-2">
                    {currentAnimGuess.name}
                  </div>
                  <div className="text-lg text-yellow-200">
                    押注：{currentAnimGuess.gender}
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="grid gap-3 mt-3">
              {!siteConfig.actualGender ? (
                <div className="bg-white/10 dark:bg-slate-900/60 border border-[var(--color-glass-border)] rounded-2xl p-4 leading-relaxed text-[var(--color-muted)]">
                  {isGambling
                    ? "盤口尚未揭曉（請在上方設置實際寶寶性別，即可自動篩選猜對玩家池）"
                    : "尚未設定實際性別，因此目前無法產生猜對名單。"}
                </div>
                ...
                correctGuesses.map((item, idx) => (
                  <div
                    key={item.id}
                    className="bg-white/10 dark:bg-slate-900/60 border border-[var(--color-glass-border)] rounded-2xl p-4 leading-relaxed"
                  >
                    <strong>
                      {idx + 1}. {item.name}
                    </strong>
          ...
          <div>
            <div
              className={`inline-flex items-center px-3 py-2 rounded-full text-[13px] font-extrabold ${isGambling ? "bg-amber-500/10 text-amber-400 border border-amber-500/25" : "bg-[rgba(140,111,232,.12)] text-[var(--color-primary-dark)] border border-[rgba(140,111,232,.15)]"}`}
            >
          ...
          <table className="w-full border-collapse min-w-[980px]">
            <thead className="sticky top-0 bg-[var(--color-bg1)] z-10">
              <tr className="border-b border-[var(--color-glass-border)]">
                <th className="p-3.5 text-left text-[13px] font-extrabold text-[var(--color-primary-dark)]">
                  #
                </th>
                <th className="p-3.5 text-left text-[13px] font-extrabold text-[var(--color-primary-dark)]">
                  玩家姓名
              ...
              {guesses.length === 0 ? (
                ...
                guesses.map((item, idx) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="border-b border-[var(--color-glass-border)] hover:bg-white/5 transition-colors"
                    >
                      <td className="p-3.5 align-top">{idx + 1}</td>
                      <td className="p-3.5 align-top font-bold">
                        {escapeHtml(item.name || "")}
                      </td>
                      <td className="p-3.5 align-top break-all">
                        {escapeHtml(item.contact || "")}
                      </td>
                      <td
                        className={`p-3.5 align-top font-extrabold ${item.gender === "男寶" ? "text-sky-400 dark:text-sky-300" : "text-pink-400 dark:text-pink-300"}`}
                      >
                        {escapeHtml(item.gender || "")}
                      </td>
                      <td className="p-3.5 align-top max-w-[200px] break-words text-sm">
                        {escapeHtml(item.wish || "")}
                      </td>
                      <td className="p-3.5 align-top text-xs font-mono">
                        {formatDate(item.createdAt)}
                      </td>
                      <td className="p-3.5 align-top font-bold">
                        {isCorrect === null ? (
                      ...
                      <td className="p-3.5 align-top">
                        <button
                          onClick={() => requestDeleteGuess(item.id!)}
                          className="bg-gradient-to-br from-[#ef5d7a] to-[#ff7f99] text-white px-3 py-1.5 rounded-full text-xs font-bold hover:scale-105 active:scale-95 transition-all"
                        >
                          刪除
                        </button>
                      </td>
      ...
      {toast && (
        <div
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-5 py-3.5 rounded-2xl border shadow-2xl animate-bounce"
          style={{
      ...
      {resetCodeConfirmId && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-[var(--color-glass-border)] rounded-3xl max-w-md w-full p-6 shadow-2xl text-slate-800 dark:text-slate-100">
            ...
            <p className="text-[var(--color-muted)] leading-relaxed mb-6">
              確定要將邀請碼{" "}
              <span className="font-mono font-bold">{resetCodeConfirmId}</span>{" "}
              恢復為未使用狀態嗎？這並不會刪除對應的歷史單。
            </p>
            <div className="flex justify-end gap-3.5">
              <button
