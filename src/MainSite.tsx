...
export default function MainSite({ themeId, setThemeId }: MainSiteProps) {
  ...
  >("hidden");
  const showOnlyReveal =
    revealState !== "finished" && !!siteConfig.actualGender;
  const [rollingName, setRollingName] = useState<string>("");
  const [revealedGenderFlashing, setRevealedGenderFlashing] =
    useState<string>("男寶");
  ...
  const handleStartReveal = () => {
    ...
    setTimeout(() => {
      ...
      const interval = setInterval(() => {
        setRevealedGenderFlashing((prev) =>
          prev === "男寶" ? "女寶" : "男寶",
        );
      }, 150);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          document
            .getElementById("lucky-draw-container")
  ...
  const handleStartDraw = (isAuto = false) => {
    ...
    const correctCandidates = guesses.filter(
      (g) => g.gender === siteConfig.actualGender,
    );
    ...
    const interval = setInterval(() => {
      const randomCandidate =
        correctCandidates[Math.floor(Math.random() * correctCandidates.length)];
  ...
  const submitVote = async (e: React.FormEvent) => {
    ...
    try {
      const q = query(
        ...
        where("contact", "==", formData.contact.trim()),
      );
      ...
      if (!snap.empty) {
        setSubmitMessage({
          type: "error",
          text: "這個聯絡方式已經參加過囉～若要修改資料，請聯絡主辦人 💜",
        });
        setSubmitting(false);
        return;
      }
      ...
      setSubmitMessage({
        ...
        text: (
          <>
            已成功送出 💜
            <br />
            <strong>{formData.name}</strong> 的猜測是：
  ...
  const boyPercent = stats.total
    ? Math.round((stats.boy / stats.total) * 100)
    : 50;
  const girlPercent = stats.total ? 100 - boyPercent : 50;

  const renderPhase2 = (isDarkModal: boolean) => (
    <div
      className={`animate-[fadeIn_0.6s_ease-out_forwards] border-t pt-8 mt-4 text-left ${isDarkModal ? "border-white/10" : "border-[rgba(140,111,232,.15)]"}`}
    >
      {/* Correct Guessers List */}
      <div className="mb-8">
        <h4
          className={`text-base sm:text-lg font-extrabold mb-4 flex items-center gap-2 ${isDarkModal ? "text-white" : "text-[var(--color-primary-dark)]"}`}
        >
          <span>🎯</span>
          <span>
            猜中正確性別的玩家名單 (
            {guesses.filter((g) => g.gender === siteConfig.actualGender).length}{" "}
            人)：
          </span>
        </h4>
        {guesses.filter((g) => g.gender === siteConfig.actualGender).length ===
        0 ? (
          <p className="text-[var(--color-muted)] text-sm font-semibold italic">
            目前沒有人猜中這個性別喔 🧩
          </p>
        ) : (
          <div
            className={`flex flex-wrap gap-2 max-h-[140px] overflow-y-auto p-4 border rounded-2xl ${isDarkModal ? "bg-white/5 border-white/10" : "bg-white/10 dark:bg-slate-900/50 border-[var(--color-glass-border)]"}`}
          >
            {guesses
              .filter((g) => g.gender === siteConfig.actualGender)
              .map((g) => (
                <div
                  key={g.id}
                  className={`px-3.5 py-1.5 rounded-full text-xs font-bold border shadow-sm flex items-center gap-1.5 ${isDarkModal ? "bg-white/10 border-white/20 text-white" : "bg-white dark:bg-slate-800 border-[var(--color-glass-border)] text-[var(--color-primary-dark)]"}`}
                >
                  <span>👶</span>
                  <span>{g.name}</span>
                </div>
      ...
      {/* Lucky Winner Drawer */}
      <div
        id="lucky-draw-container"
        className={`border rounded-3xl p-6 md:p-8 shadow-inner scroll-mt-24 ${isDarkModal ? "border-white/10 bg-white/5" : "border-[var(--color-glass-border)] bg-gradient-to-br from-[var(--color-glass-bg)] to-white/5"}`}
      >
        <div className="text-center max-w-xl mx-auto">
          ...
          <h4
            className={`text-xl sm:text-2xl font-black mb-2 ${isDarkModal ? "text-white" : "text-[var(--color-primary-dark)]"}`}
          >
            幸運得主大抽獎
          </h4>
          <p
            className={`text-xs sm:text-sm font-semibold leading-relaxed mb-6 ${isDarkModal ? "text-gray-300" : "text-[var(--color-muted)]"}`}
          >
          ...
          {drawState === "ready" && drawCountdown === null && (
            <div
              className={`border rounded-2xl p-6 mb-6 ${isDarkModal ? "bg-white/10 border-white/20" : "bg-white/10 dark:bg-slate-900/60 border-[var(--color-glass-border)]"}`}
            >
              <div className="text-[36px] sm:text-[48px] filter saturate-50 animate-pulse mb-3">
                🎰
              </div>
              <p
                className={`text-sm font-extrabold mb-4 ${isDarkModal ? "text-gray-300" : "text-[var(--color-muted)]"}`}
              >
                準備好揭曉幸運得獎者了嗎？點擊下方按鈕啟動轉輪！
              </p>
              <button
              ...
              {(!siteConfig.winners || siteConfig.winners.length === 0) && (
                <p className="text-xs text-red-500 font-bold mt-2.5">
                  ⚠️
                  主辦人尚未在後台完成隨機抽獎，暫時無法啟動開獎機。請靜待通知！
                </p>
              )}
            </div>
          )}
          ...
          {drawState === "drawing" && (
            <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 border-4 border-yellow-400 rounded-2xl p-8 mb-6 shadow-2xl relative overflow-hidden animate-[pulse_1s_infinite]">
              ...
              <div className="text-yellow-400 text-xs font-mono tracking-widest uppercase mb-2 animate-pulse">
                ● SPINNER ACTIVE
              </div>
              <div className="text-3xl sm:text-5xl font-black text-white font-mono tracking-wider animate-bounce">
                {rollingName || "???"}
              </div>
              <p className="text-indigo-200/80 text-xs font-bold mt-4 animate-pulse">
                正在核對全場對中盤口數據，計算極致好運得主...
              </p>
            </div>
          )}
          ...
          {drawState === "done" && (
            <div>
              <div
                className={`border-2 rounded-2xl p-6 sm:p-8 mb-6 shadow-xl text-center ${isDarkModal ? "bg-yellow-950/20 border-yellow-900" : "bg-gradient-to-r from-amber-50/60 to-yellow-50/60 dark:from-yellow-950/10 dark:to-slate-900/50 border-yellow-300 dark:border-yellow-900/50"}`}
              >
                <div className="text-5xl mb-4 animate-bounce">👑</div>
                <h5 className="text-2xl font-extrabold text-amber-700 dark:text-amber-400 mb-4">
                  🏆 恭喜以下幸運中獎者 🏆
                </h5>
                ...
                <div className="grid gap-4 max-w-lg mx-auto">
                  {!siteConfig.winners || siteConfig.winners.length === 0 ? (
                    <p
                      className={`text-sm font-semibold italic ${isDarkModal ? "text-gray-400" : "text-[var(--color-muted)]"}`}
                    >
                      暫無得獎者數據 📭
                    </p>
                  ) : (
                    siteConfig.winners.map((item, idx) => (
                      <div
                        key={item.id || idx}
                        className={`border rounded-xl p-4 text-left shadow-sm ${isDarkModal ? "bg-slate-900/80 border-yellow-900/50 text-gray-200" : "bg-white dark:bg-slate-800 border-yellow-200 dark:border-yellow-900/40 text-[var(--color-muted)]"}`}
                      >
                        <div
                          className={`flex justify-between items-start gap-2 border-b pb-2 mb-2 ${isDarkModal ? "border-slate-700" : "border-gray-100 dark:border-slate-700"}`}
                        >
                          <strong
                            className={`text-base ${isDarkModal ? "text-white" : "text-[var(--color-primary-dark)]"}`}
                          >
  ...
  return (
    <>
      <div className="floating-bg">
        <div
          className="bubble b1"
          style={{ color: isGambling ? "rgba(251, 191, 36, 0.15)" : undefined }}
        >
          $
        </div>
        <div
      ...
      {!hasPublishedGender && (
        <div
          ...
          style={{
            backgroundImage: `url(${(import.meta as any).env.BASE_URL || ""}casino.png)`,
          }}
        >
          ...
          <div
            ...
            onClick={() =>
              document
                .getElementById("main-content-start")
          ...
          >
            ...
            <svg
              className="w-6 h-6 text-amber-400 animate-bounce drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <header className="py-4 md:py-5 w-[min(1180px,calc(100%-24px))] mx-auto relative z-10">
        <div
          className="shadow-[var(--shadow-custom)] backdrop-blur-[14px] rounded-3xl md:rounded-full px-4 py-3 md:px-[18px] md:py-[14px] flex flex-col md:flex-row justify-between items-center gap-3 md:gap-[14px]"
          style={{
        ...
        >
          <div className="flex items-center gap-2.5 sm:gap-3 font-extrabold text-[var(--color-primary-dark)] tracking-wider text-sm sm:text-base">
            <div
              className={`w-[36px] h-[36px] sm:w-[42px] sm:h-[42px] rounded-full grid place-items-center ${isGambling ? "bg-gradient-to-br from-[#ffe066] to-[#ffaa00] shadow-[0_6px_16px_rgba(251,191,36,0.35)]" : "bg-gradient-to-br from-[#c9b3ff] to-[#ffd8ee] shadow-[0_6px_16px_rgba(140,111,232,.18)]"} text-[16px] sm:text-[20px]`}
            >
              🍼
            </div>
            <div>
              {isGambling ? "🎰 澱粉寶寶皇家娛樂城" : "澱粉寶寶皇家娛樂城"}
            </div>
          </div>
          <nav className="flex gap-1 sm:gap-2 flex-wrap items-center justify-center">
            ...
            {isAdmin(currentUser?.email) ? (
              <Link
                to="/admin"
                className={`no-underline text-[var(--color-text)] font-bold text-xs sm:text-sm px-2.5 py-1.5 sm:px-3.5 sm:py-2 rounded-full transition-colors ${isGambling ? "hover:bg-amber-500/10 hover:text-amber-400" : "hover:bg-[rgba(140,111,232,.12)] hover:text-[var(--color-primary-dark)]"}`}
              >
                管理後台
              </Link>
            ) : (
      ...
      <main>
        ...
        {hasPublishedGender && (
          <section
            id="reveal"
            className="py-[26px] w-[min(1180px,calc(100%-32px))] mx-auto relative z-10 scroll-mt-6"
          >
            <div
              className="border shadow-[var(--shadow-custom)] rounded-[28px] p-6 md:p-10 text-center relative overflow-hidden"
              style={{
                background: "var(--color-glass-bg)",
                borderColor: "var(--color-glass-border)",
                borderWidth: "1px",
            ...
            >
              {/* Header Title */}
              <div
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-extrabold mb-5 ${isGambling ? "bg-amber-500/10 border border-amber-500/25 text-amber-400 shadow-[0_2px_8px_rgba(251,191,36,0.15)]" : "bg-[rgba(140,111,232,.12)] border border-[rgba(140,111,232,.15)] text-[var(--color-primary-dark)]"}`}
              >
                {isGambling
                  ? "🎰 澱粉寶寶現場即時派彩盤口 👑"
                  : "🎉 期待已久的揭曉時刻 👶"}
                  ...
                  <div className="reveal-container fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-start min-h-screen overflow-y-auto overflow-x-hidden select-none pt-12 sm:pt-28 md:pt-36 lg:pt-40 pb-12 sm:pb-24 text-white dark">
                    ...
                    revealState === "safe_opening" ? (
                      ...
                      <>
                        ...
                        <div
                          className={`absolute inset-0 transition-all duration-1000 z-0 blur-3xl ${
                            revealState === "revealed"
                              ? "opacity-40 " +
                                (siteConfig.actualGender === "男寶"
                                  ? "bg-gradient-to-tr from-blue-600 via-indigo-600 to-transparent"
                                  : "bg-gradient-to-tr from-pink-600 via-rose-600 to-transparent")
                              : "opacity-25 " +
                        ...
                        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] sm:w-[460px] aspect-square pointer-events-none z-10">
                          ...
                          <div
                            className={`absolute inset-0 transition-opacity duration-1000 ${revealState === "revealed" ? "opacity-0" : "opacity-100"}`}
                          >
                            <div
                              className="absolute left-1/2 top-1/2 border-4 border-dashed border-amber-400/40 rounded-full w-full h-full"
                              style={{
                                animation:
                                  "fgoSpinClockwise 9.0s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards",
                              }}
                            >
                            ...
                            <div
                              ...
                              style={{
                                animation:
                                  "fgoSpinCounterClockwise 8.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards",
                              }}
                            >
                          ...
                          <div
                            className={`absolute inset-0 transition-opacity duration-1000 ${revealState === "revealed" ? "opacity-75" : "opacity-0"}`}
                          >
                            <div
                        ...
                        <div
                          ...
                          style={{
                            animation:
                              "fgoPillarBurst 9.0s ease-in-out forwards",
                            maskImage:
                        ...
                        <div
                          ...
                          style={{
                            animation:
                              "fgoLightningStrike 3.5s ease-in-out infinite",
                          }}
                        >
                          <svg
                            className="w-full h-full stroke-yellow-300 dark:stroke-cyan-300 opacity-60"
                            viewBox="0 0 100 100"
                            preserveAspectRatio="none"
                          >
                            <path
                              d="M 50 0 L 48 25 L 53 45 L 46 65 L 51 100"
                              fill="none"
                              strokeWidth="0.6"
                            />
                            <path
                              d="M 25 5 L 28 35 L 24 55 L 27 100"
                              fill="none"
                              strokeWidth="0.4"
                              className="hidden sm:block"
                            />
                            <path
                              d="M 75 10 L 72 40 L 76 60 L 73 100"
                              fill="none"
                              strokeWidth="0.4"
                              className="hidden sm:block"
                            />
                        ...
                        <div
                          ...
                          style={{
                            animation:
                              revealState === "revealed"
                                ? "fgoCardHover 4s ease-in-out infinite"
                                : "fgoCardFloatIn 11.0s ease-out forwards",
                            perspective: "1000px",
                          }}
                        >
                          <div
                            ...
                            style={{
                              ...
                              animation:
                                revealState === "revealed"
                                  ? "none"
                                  : "fgoCardSpinRoulette 11.0s linear forwards",
                              transform:
                                revealState === "revealed"
                                  ? "rotateY(180deg)"
                                  : undefined,
                            ...
                            >
                              ...
                              <img
                                ...
                                style={{
                                  animation:
                                    revealState === "revealed"
                                      ? "none"
                                      : "girlFlash 10.0s linear forwards",
                                  opacity: revealState === "revealed" ? 0 : 1,
                                }}
                                referrerPolicy="no-referrer"
                              />
                              <img
                                ...
                                style={{
                                  animation:
                                    revealState === "revealed"
                                      ? "none"
                                      : "finalFlash 10.0s linear forwards",
                                  opacity: revealState === "revealed" ? 1 : 0,
                                }}
                                referrerPolicy="no-referrer"
                              />
                              {revealState === "revealing" && (
                                <div
                                  ...
                                  style={{
                                    animation:
                                      "whiteFlash 10.0s linear forwards",
                                  }}
                                />
                              )}
                            </div>
                        ...
                        {/* Bottom Status panel */}
                        <div className="relative z-20 mt-2 landscape:mt-1 sm:mt-8 text-center w-full max-w-md shrink-0 min-h-[160px]">
                          <div
                            className={`transition-opacity duration-500 absolute w-full left-0 top-0 ${revealState === "revealing" ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                          >
                          ...
                          <div
                            className={`transition-opacity duration-1000 delay-300 ${revealState === "revealed" ? "opacity-100 relative" : "opacity-0 pointer-events-none absolute w-full left-0 top-0"}`}
                          >
                            <div className="text-sm font-extrabold text-amber-400 mb-2 uppercase tracking-widest font-mono">
                              🏆 SUMMON REVEAL SUCCESS 🏆
                            </div>
                            <h3
                              className={`text-3xl sm:text-4xl font-black tracking-wider mb-3 filter drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)] ${
                                siteConfig.actualGender === "男寶"
                                  ? "text-cyan-300"
                                  : "text-pink-300"
                              }`}
                            ...
                            <p className="text-gray-300 text-xs sm:text-sm font-semibold leading-relaxed bg-black/60 border border-indigo-950 px-6 py-3.5 rounded-2xl shadow-xl max-w-xs sm:max-w-none mx-auto text-center">
                              {siteConfig.actualGender === "男寶"
                                ? isGambling
                                  ? "🎰 買中藍方【男寶】的玩家獲得全額 1.95 派彩彩金！全場沸騰中！💸"
                                  : "恭喜所有猜測【男寶】的親朋好友們，你們太厲害、太神準了！🎉"
                                : isGambling
                                  ? "🎰 買中紅方【女寶】的玩家獲得全額 1.95 派彩彩金！全場沸騰中！💸"
                                  : "恭喜所有猜測【女寶】的親朋好友們，你們太厲害、太神準了！🎉"}
                            </p>
                          </div>
                        </div>
              ...
              {(revealState === "revealed" || revealState === "initial") && (
                <div className="fixed bottom-4 right-4 z-[110]">
                  <Link
                    to="/admin"
                    className="text-white/20 hover:text-white/70 text-xs font-mono transition-colors"
                  >
                    管理後台
                  </Link>
                </div>
              ...
              {revealState === "finished" && (
                <div className="mt-10 p-6 md:p-8 rounded-[24px] bg-slate-950 text-white shadow-2xl relative overflow-hidden border border-indigo-900/30 text-left">
                  ...
                  <div className="relative z-10">{renderPhase2(true)}</div>
                </div>
              )}
            </div>
          </section>
        ...
        {!hasPublishedGender && !showOnlyReveal && (
          <>
            ...
            <section
              id="countdown-scrolled"
              className="py-8 pb-4 w-[min(1180px,calc(100%-32px))] mx-auto relative z-10 text-center scroll-mt-24"
            >
              <div
                className="rounded-[var(--radius-xl)] p-8 md:p-12 border border-[var(--color-glass-border)] shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden flex flex-col items-center gap-6 animate-pulse-custom"
                style={{ background: "var(--color-card-grad)" }}
              >
                ...
                <div className="relative z-10 mt-2 w-full max-w-xl mx-auto">
                  ...
                  <div className="grid grid-cols-4 gap-3 sm:gap-4">
                    <div className="bg-black/65 backdrop-blur-md border border-yellow-500/35 rounded-2xl p-3 sm:p-5 text-center shadow-[0_8px_32px_rgba(184,134,11,0.18)]">
                      <div className="text-[var(--color-primary)] text-2xl sm:text-4xl font-black font-mono tracking-tight">
                        {timeLeft.days}
                      </div>
                      <div className="text-amber-200/80 text-[10px] sm:text-xs font-extrabold mt-1">
                        天
                      </div>
                    </div>
                    <div className="bg-black/65 backdrop-blur-md border border-yellow-500/35 rounded-2xl p-3 sm:p-5 text-center shadow-[0_8px_32px_rgba(184,134,11,0.18)]">
            ...
            <section className="py-4 pb-6 w-[min(1180px,calc(100%-32px))] mx-auto relative z-10">
              <div
                className="rounded-[var(--radius-xl)] p-6 md:p-10 border border-[var(--color-glass-border,rgba(255,255,255,0.8))] shadow-[var(--shadow-custom)] grid grid-cols-1 lg:grid-cols-[1.15fr_.85fr] gap-6 items-center overflow-hidden relative"
                style={{ background: "var(--color-card-grad)" }}
              >
                <div>
                  <div
                    className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-extrabold mb-[18px] ${isGambling ? "bg-amber-500/10 border border-amber-500/25 text-amber-400 shadow-[0_2px_8px_rgba(251,191,36,0.15)]" : "bg-[rgba(140,111,232,.12)] border border-[rgba(140,111,232,.15)] text-[var(--color-primary-dark)]"}`}
                  >
                    {isGambling
                      ? "🔥 全台最大盤口熱烈開盤中・買定離手！"
                      : "✨ 一起來猜猜看，寶寶到底是男生還是女生？"}
                  ...
                  <p className="text-[var(--color-muted)] leading-[1.9] text-base mb-6 font-semibold">
                    {isGambling
                      ? "【賠率全新升級：1 賠 1.95】支持心中所屬陣營，不需儲值，填寫祝福直接免費領取下注金！猜對在性別正式揭曉派彩日即有機會抽出極奢大禮！🎁"
                      : "我們想把迎接寶寶的喜悅分享給每一位家人朋友～在正式揭曉前，先來玩一個小小的猜謎活動吧！猜對的人將有機會獲得小禮物 🎁"}
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-3.5">
                    <div
                      className="border shadow-[0_8px_20px_rgba(120,93,200,.08)] rounded-[16px] sm:rounded-[18px] px-2.5 py-3 sm:px-4 sm:py-3.5 text-center sm:text-left"
                      style={{
                        background: "var(--color-glass-bg)",
                        borderColor: "var(--color-glass-border)",
                        borderWidth: "1px",
                    ...
                    >
                      <div className="text-[var(--color-muted)] text-[11px] sm:text-[13px] font-bold mb-1 sm:mb-1.5 break-all">
                        {isGambling ? "🎰 總下注人次" : "目前參加人數"}
                      </div>
                      <div className="text-[18px] sm:text-[24px] font-extrabold text-[var(--color-primary-dark)]">
                        {stats.total}
                      </div>
                    </div>
                    <div
                    ...
                    >
                      <div className="text-[var(--color-muted)] text-[11px] sm:text-[13px] font-bold mb-1 sm:mb-1.5 break-all">
                        {isGambling ? "💙 押注男寶金" : "男寶票數"}
                      </div>
                      <div className="text-[18px] sm:text-[24px] font-extrabold text-[var(--color-primary-dark)]">
                        {isGambling ? `${stats.boy * 1000} 萬` : stats.boy}
                      </div>
                    </div>
                    <div
                      className="border shadow-[0_8px_20px_rgba(120,93,200,.08)] rounded-[16px] sm:rounded-[18px] px-2.5 py-3 sm:px-4 sm:py-3.5 text-center sm:text-left"
                    ...
                    >
                      <div className="text-[var(--color-muted)] text-[11px] sm:text-[13px] font-bold mb-1 sm:mb-1.5 break-all">
                        {isGambling ? "💖 押注女寶金" : "女寶票數"}
                      </div>
                      <div className="text-[18px] sm:text-[24px] font-extrabold text-[var(--color-primary-dark)]">
                        {isGambling ? `${stats.girl * 1000} 萬` : stats.girl}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="min-h-[300px] sm:min-h-[360px] lg:min-h-[400px] flex items-center justify-center relative">
                  <div
                    className="w-[min(100%,430px)] aspect-square rounded-[32px] border border-[var(--color-glass-border,rgba(255,255,255,0.85))] shadow-[var(--shadow-custom)] flex items-center justify-center relative overflow-hidden"
                    style={{ background: "var(--color-card-grad)" }}
                  >
            ...
            >
              <div className="text-center mb-6">
                <div
                  className={`inline-block px-3.5 py-2 rounded-full text-[13px] font-extrabold mb-3 ${isGambling ? "bg-amber-500/10 text-amber-400 border border-amber-500/20 shadow-sm" : "bg-[rgba(140,111,232,.12)] text-[var(--color-primary-dark)]"}`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4.5 mt-7">
                ...
                >
                  <div
                    className={`w-[54px] h-[54px] rounded-[18px] grid place-items-center mb-3.5 text-2xl font-black ${isGambling ? "bg-gradient-to-br from-[#ffd700] to-[#ffaa00] text-amber-950 shadow-[0_4px_12px_rgba(251,191,36,0.35)]" : "bg-gradient-to-br from-[rgba(205,187,255,.68)] to-[rgba(255,216,238,.7)]"}`}
                  >
                    1️⃣
                  </div>
                  ...
                  <p className="text-[var(--color-muted)] leading-[1.85] text-[15px] font-semibold">
                    {isGambling
                      ? "點選您看好的寶寶性別陣營（男寶/女寶），目前雙向賠率全面升級為 1:1.95！"
                      : "選擇你猜測的寶寶性別，可以是「男寶」或「女寶」。"}
                  </p>
                </div>
                <div
                  className={`border shadow-[var(--shadow-custom)] rounded-[24px] p-6 relative overflow-hidden ${isGambling ? "after:bg-[radial-gradient(circle,rgba(251,191,36,0.15),transparent)]" : "after:bg-[radial-gradient(circle,rgba(205,187,255,.25),transparent)]"} after:content-[''] after:absolute after:-right-[18px] after:-top-[18px] after:w-[90px] after:h-[90px] after:rounded-full`}
                ...
                >
                  ...
                  <p className="text-[var(--color-muted)] leading-[1.85] text-[15px] font-semibold">
                    {isGambling
                      ? "填寫正確姓名與LINE或手機聯絡資訊，並留下最誠摯的祝福，不需資金即可免費送出下注！"
                      : "方便我們在揭曉後通知猜對的朋友，也可以留下對寶寶或爸媽的祝福。"}
                  </p>
                </div>
                <div
                  className={`border shadow-[var(--shadow-custom)] rounded-[24px] p-6 relative overflow-hidden ${isGambling ? "after:bg-[radial-gradient(circle,rgba(251,191,36,0.15),transparent)]" : "after:bg-[radial-gradient(circle,rgba(205,187,255,.25),transparent)]"} after:content-[''] after:absolute after:-right-[18px] after:-top-[18px] after:w-[90px] after:h-[90px] after:rounded-full`}
                ...
                >
                  ...
                  <p className="text-[var(--color-muted)] leading-[1.85] text-[15px] font-semibold">
                    {isGambling
                      ? "截止日一到立即封盤！寶寶正式誕生揭曉後，平台結算從猜對的投注單中抽出尊榮大獎！"
                      : "活動截止後不再接受投票，等寶寶性別揭曉後，會從猜對的人中抽出幸運得主！"}
                  </p>
                </div>
              </div>
            </section>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-6 mt-7">
                <div
                  className="border border-[var(--color-glass-border,rgba(255,255,255,0.85))] shadow-[var(--shadow-custom)] rounded-[28px] p-5 sm:p-[26px]"
                  style={{ background: "var(--color-glass-bg)" }}
                >
                  <div className="flex justify-between items-center gap-3 mb-4.5">
                    ...
                    <div
                      className={`px-3 py-2 rounded-full text-[13px] font-extrabold leading-none ${isGambling ? "bg-amber-500/10 border border-amber-500/25 text-amber-400" : "bg-[rgba(140,111,232,.12)] border border-[rgba(140,111,232,.15)] text-[var(--color-primary-dark)]"}`}
                    >
                      即時賠率統計
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4.5">
                    <label
                      className={`relative rounded-[24px] px-[18px] py-[22px] min-h-[210px] cursor-pointer transition-all border-2 flex flex-col justify-between overflow-hidden ${
                        isGambling
                    ...
                    >
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-3.5">
                          ...
                          <div
                            className={`px-3 py-2 rounded-full text-[13px] font-extrabold leading-none ${isGambling ? "bg-amber-400 text-amber-950" : "bg-white/80 dark:bg-slate-800 text-[var(--color-primary-dark)]"}`}
                          >
                        ...
                        <p className="text-[var(--color-muted)] leading-[1.75] text-sm">
                          {isGambling
                            ? "【極速開盤】帥氣小王子來臨？看好小男生陽光帥氣路線？速押男寶！"
                            : "陽光小男孩報到？你覺得這次是帥氣小王子路線嗎 👶"}
                        </p>
                      </div>
                    </label>
                    ...
                    >
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-3.5">
                          ...
                          <div
                            className={`px-3 py-2 rounded-full text-[13px] font-extrabold leading-none ${isGambling ? "bg-pink-400 text-pink-950" : "bg-white/80 dark:bg-slate-800 text-[var(--color-primary-dark)]"}`}
                          >
                        ...
                        <p className="text-[var(--color-muted)] leading-[1.75] text-sm">
                          {isGambling
                            ? "【熱烈推薦】可愛小公主登場？感受滿滿粉嫩甜蜜的氣息？速押女寶！"
                            : "甜甜小公主登場？你是不是已經感受到滿滿可愛氣息了 🎀"}
                        </p>
                      </div>
                    </label>
                  </div>
                  ...
                  <div
                    className={`mt-4.5 border rounded-[18px] p-4 backdrop-blur-sm ${
                      isGambling
                  ...
                  >
                    <div className="flex justify-between gap-2.5 mb-3 text-[var(--color-primary-dark)] text-sm font-extrabold">
                      <span>
                        {isGambling ? "📈 兩側池子投注比例" : "目前投票比例"}
                      </span>
                      <span>
                        男寶 {boyPercent}% / 女寶 {girlPercent}%
                      </span>
                    </div>
                    <div
                      className={`h-[14px] rounded-full overflow-hidden flex ${
                        isGambling
                          ? "bg-amber-950/50"
                          : "bg-[#f0e9ff] dark:bg-slate-850"
                      }`}
                    >
                      <div
                        className="h-full bg-gradient-to-r from-[#00bfff] to-[#00f0ff] transition-all duration-300"
                        style={{ width: `${boyPercent}%` }}
                      ></div>
                      <div
                        className="h-full bg-gradient-to-r from-[#ff007f] to-[#ff66b2] transition-all duration-300"
                        style={{ width: `${girlPercent}%` }}
                      ></div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                      <div
                        className={`rounded-[16px] py-3 px-3.5 text-center text-sm font-extrabold text-[var(--color-primary-dark)] border ${
                          isGambling
                            ? "bg-amber-950/20 border-sky-500/20"
                      ...
                      <div
                        className={`rounded-[16px] py-3 px-3.5 text-center text-sm font-extrabold text-[var(--color-primary-dark)] border ${
                          isGambling
                            ? "bg-amber-950/20 border-pink-500/20"
                ...
                >
                  ...
                  <form
                    onSubmit={submitVote}
                    className={isClosed ? "opacity-70 pointer-events-none" : ""}
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <div className="flex flex-col gap-2 mb-3.5">
                        <label
                          htmlFor="name"
                          className="text-sm font-extrabold text-[var(--color-primary-dark)]"
                        >
                          {isGambling
                            ? "下注玩家姓名 / 暱稱 *"
                            : "姓名 / 暱稱 *"}
                        ...
                        <input
                          ...
                          value={formData.name}
                          onChange={handleInputChange}
                          className={`w-full bg-white/10 rounded-[16px] px-4 py-3.5 text-[15px] text-[var(--color-text)] outline-none transition-all focus:border-[var(--color-primary)] ${isGambling ? "border border-amber-500/20 focus:shadow-[0_0_0_4px_rgba(251,191,36,0.2)]" : "border border-[rgba(140,111,232,.2)] dark:bg-slate-900/60 focus:shadow-[0_0_0_4px_rgba(140,111,232,.15)]"}`}
                          type="text"
                          placeholder="例如：玩家小龍 / 賭神高進"
                          required
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-2 mb-3.5 col-span-1 sm:col-span-2">
                        ...
                        <textarea
                          ...
                          value={formData.wish}
                          onChange={handleInputChange}
                          className={`w-full bg-white/10 rounded-[16px] px-4 py-3.5 text-[15px] text-[var(--color-text)] outline-none transition-all focus:border-[var(--color-primary)] min-h-[120px] resize-y leading-[1.7] ${isGambling ? "border border-amber-500/20 focus:shadow-[0_0_0_4px_rgba(251,191,36,0.2)]" : "border border-[rgba(140,111,232,.2)] dark:bg-slate-900/60 focus:shadow-[0_0_0_4px_rgba(140,111,232,.15)]"}`}
                          placeholder="例如：恭喜當爸爸媽媽！祝寶寶健康快樂，爆發超凡好運 💜"
                        ></textarea>
                      </div>
                    </div>

                    <div
                      className={`mt-2.5 rounded-[16px] px-4 py-3.5 text-[var(--color-muted)] text-[13px] leading-[1.8] border ${isGambling ? "bg-amber-500/5 border-amber-500/20" : "bg-[rgba(140,111,232,.08)] border-[rgba(140,111,232,.15)]"}`}
                    >
                      {isGambling ? (
                        <>
                          📌 <strong>🎰 莊家風控及投注規章：</strong>
                          <br />
                          1.
                          每位玩家（單一裝置）限制投單一次，嚴禁多開惡意刷單。
                          <br />
                          2. 封盤倒數歸零後盤口即刻封死，概不受理任何補單 or
                          改單。
                          <br />
                          3. 本次活動為 100%
                          免費娛樂，不涉及任何真實金錢投注，請放心遊玩！
                          <br />
                          4.
                          派彩大抽獎及最終獎品發放規則，最終解釋權歸莊家（爸媽）所有！
                        </>
                      ) : (
                        <>
                          📌 <strong>活動規則提醒：</strong>
                          <br />
                          1. 每人原則上填寫一次。
                          <br />
                          2. 活動截止後將不再受理修改。
                    ...
                    <div className="flex gap-3 flex-wrap items-center mt-4.5">
                      <button
                        type="submit"
                        disabled={submitting}
                        className={`inline-flex items-center justify-center gap-2 px-[22px] py-[14px] rounded-full text-[15px] font-extrabold transition-all w-full sm:w-auto ${isGambling ? "bg-gradient-to-r from-[#ffd700] via-[#ffaa00] to-[#b8860b] text-black shadow-[0_12px_28px_rgba(184,134,11,0.4)] hover:shadow-[0_16px_32px_rgba(184,134,11,0.6)]" : "text-white bg-gradient-to-br from-[var(--color-primary)] to-[#ab90ff] shadow-[0_12px_28px_rgba(140,111,232,.28)] hover:shadow-[0_16px_32px_rgba(140,111,232,.34)]"} hover:-translate-y-0.5 ${submitting ? "opacity-70 cursor-not-allowed hover:translate-y-0" : ""}`}
                      >
                        {isClosed
                          ? isGambling
                            ? "⚠️ 已截止封盤"
                      ...
                      {submitting && (
                        <div className="inline-flex items-center gap-2 text-[var(--color-muted)] text-sm font-bold">
                          ⏳ 投單同步至雲端伺服器中...
                        </div>
                      )}
                    </div>
                    ...
                    {submitMessage && (
                      <div
                        className={`mt-4.5 px-[18px] py-[16px] rounded-[18px] leading-[1.8] font-bold ${
                ...
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1.1fr_.9fr] gap-6 items-center">
                  <div>
                    ...
                    <p className="leading-[1.95] opacity-95 text-[15px] font-medium">
                      {isGambling ? (
                        ...
                        <>
                          為了讓這份等待更有參與感，我們準備了一份精緻的喵喵伴手禮給猜對的朋友。
                          <br />
                          <br />
                          等到寶寶性別正式揭曉後，會從{" "}
                          <strong>猜對答案的名單中抽出幸運得主</strong>
                          ，並用你留下的聯絡方式通知你。謝謝你一起參與這個可愛又充滿期待的小活動
                          💜
                        </>
                      )}
                    </p>
                  ...
                  <div className="grid gap-3">
                    <div className="bg-white/15 border border-white/20 rounded-[18px] px-4 py-3.5 leading-[1.75] backdrop-blur-[8px]">
                      {isGambling ? (
                        <>
                          🎟️ <strong>派彩條件：</strong>
                          活動截止前成功送出下注，並猜中寶寶正確性別。
                        </>
                      ) : (
                        <>
                          🎀 <strong>抽獎資格：</strong>
      ...
      {!hasPublishedGender && !showOnlyReveal && (
        <footer className="py-[30px] pb-[42px] w-[min(1180px,calc(100%-32px))] mx-auto relative z-10">
          <div
            className="border shadow-[var(--shadow-custom)] rounded-[28px] p-6 text-center"
            style={{
              background: "var(--color-glass-bg)",
              borderColor: "var(--color-glass-border)",
              borderWidth: "1px",
