...
export const CelebrationBall: React.FC<CelebrationBallProps> = ({ isOpen }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const colors = [
        '#ff4081', '#00e5ff', '#ffeb3b', '#4caf50', '#9c27b0', 
        '#ff9800', '#e91e63', '#00decb', '#ff5722', '#3f51b5'
      ];
  ...
  return (
    <div className="fixed inset-x-0 top-0 pointer-events-none z-50 overflow-visible select-none h-screen flex flex-col items-center">
      ...
      {/* Hanging rope */}
      <div 
        className="w-[3px] bg-red-600/80 shadow-[0_2px_10px_rgba(0,0,0,0.15)] origin-top"
        style={{ 
          height: '240px',
      ...
      <div className="relative -mt-1 w-0 h-0 flex items-center justify-center">
        ...
        {confetti.map((item) => (
          <div
            ...
            style={{
              ...
              top: '0px',
              backgroundColor: item.shape !== 'triangle' ? item.color : 'transparent',
              borderColor: item.shape === 'triangle' ? `transparent transparent ${item.color} transparent` : 'transparent',
              borderWidth: item.shape === 'triangle' ? `0 ${item.size / 2}px ${item.size}px ${item.size / 2}px` : '0',
              width: item.shape !== 'triangle' ? `${item.size}px` : '0',
              height: item.shape !== 'triangle' ? (item.shape === 'rect' ? `${item.size * 1.5}px` : `${item.size}px`) : '0',
        ...
        >
          ...
          <div className="text-yellow-300 text-xs font-black tracking-widest mb-1">🎁 恭喜得獎 🎁</div>
        ...
        <div 
          ...
          style={{ 
            boxShadow: '-8px 12px 24px rgba(0,0,0,0.25), inset -2px 0 8px rgba(255,220,100,0.5)'
          }}
        >
